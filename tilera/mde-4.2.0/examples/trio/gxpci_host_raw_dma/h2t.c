...
#define H2T_HUGE_PAGES		8

// The packet memory pool.
void *h2t_page[H2T_HUGE_PAGES];
...
void do_recv(gxpci_context_t* context, void* buf_mem)
{
  ...
  while (1)
  {
    ...
    {
      unsigned credits = gxpci_raw_dma_recv_get_credits(context);
      ...
      for (int i = 0; i < count; i++)
      {
        ...
        cmd[i].remote_buf_offset =
          ((recv_seq_num + i) % slots_in_host_buffer) * MAX_PKT_SIZE;
        result = gxpci_raw_dma_recv_cmd(context, &cmd[i]);
        if (result == GXPCI_ERESET)
        {
          printf("do_recv: channel is reset\n");
          goto recv_reset;
        }
        VERIFY_ZERO(result, "gxpci_raw_dma_recv_cmd()");
      }
      recv_seq_num += count;
    }
    else
    ...
    int comp_count = gxpci_raw_dma_recv_get_comps(context, comp, 0,
                                                  MAX_CMDS_BATCH);
...
recv_reset:
  ...
  gxpci_raw_dma_recv_destroy(context);
}
...
void *thread_recv(void *arg)
{
  ...
  void *buf_mem = h2t_page[queue_index / PAGE_SHARER] +
  ...
  printf("RD thread_recv(%d): queue_index %d\n", rank, queue_index);
  
  result = gxpci_open_raw_dma_recv_queue(trio_context, rd_state,
                                         gxpci_context, queue_index);
  VERIFY_ZERO(result, "gxpci_open_raw_dma_recv_queue()");

  host_buf_size = gxpci_raw_dma_get_host_buf_size(gxpci_context);

  printf("thread_recv(%d): H2T queue opened, host buffer size %#x, "
         "waiting to receive ...\n", rank, host_buf_size);
...
int main(int argc, char**argv)
{
  ...
  num_receivers = rd_state->num_rd_h2t_queues;
  ...
  //
  // Start pthreads to receive data.
  //
