...
#define EQUEUE_RSV_CHUNK 1024

//#define USE_STICKY_MODE

#define WORKER_BATCH_SIZE 64

// Burn cycles in the worker to emulate real per-packet processing time.
#define WORKER_WASTE_CYCLES 0

// In sticky mode, the idescs will burst to a single NotifRing until the 
// queue is full.  Thus the iqueue itself should be spread out in this case.
#ifndef USE_STICKY_MODE
...
static int send_queue_table[32];
...
// Up to 16 link names associated with this app
#define MAX_LINKS 16
...
// It's job is to return the output queue for the packet or DROP_QUEUE if the 
// packet should be dropped.
static __USUALLY_INLINE int
handle_packet(gxio_mpipe_idesc_t *idesc)
{
  // The queue selection is application-specific.  Typically, this would 
  // be a table lookup etc.  Just assign here in a balanced way.  
  int output_queue_select = send_queue_table[idesc->channel];

#if WORKER_WASTE_CYCLES
  uint64_t start_cyc = __insn_mfspr(SPR_CYCLE);
  while (__insn_mfspr(SPR_CYCLE) < (start_cyc + WORKER_WASTE_CYCLES));
#endif

  return output_queue_select;
}
...
// first starts up.
static int 
send_startup_packets(gxio_mpipe_equeue_t* equeue)
{
  int bstack = -1;
  ...
  for (int i = 0; i < startup_packets; i++)
  {
    ...
    for (int j = 0; j < startup_packet_size; j += 4)
      ((uint32_t *)buf)[j] = data_pattern++;
    __insn_mf();
...
main_aux_sender(void* arg)
{
  ...
  // A send tile can be responsible for more than one queue.  The min/max_qid's
  // indicate the range of quues this sender will handle.
  ...
  if (startup_packets)
  {
    ...
    slot = send_startup_packets(equeue);
  }
  ...
  for (int i = 0; i < 64; i += COMPQ_VLD_BITS)
    comp_simd |= (uint64_t)min_qid << i;
#if QUEUES_PER_SEND > 1
  uint64_t comp_simd2 = 0;
  for (int i = 0; i < 64; i += COMPQ_VLD_BITS)
    comp_simd2 |= (uint64_t)max_qid << i;
#endif
  ...
  while (workers_complete < num_workers)
  {
    ...
    while (credits >= CV_VLD_PER_WD)
    {
      ...
      uint64_t av_sel = av & ~CV_COMP_MASK;

#if QUEUES_PER_SEND == 1

#if COMPQ_VLD_BITS == 8
      uint64_t compv = __insn_v1cmpeq(av_sel, comp_simd);
#elif COMPQ_VLD_BITS == 16
      uint64_t compv = __insn_v2cmpeq(av_sel, comp_simd);
#elif COMPQ_VLD_BITS == 32
      uint64_t compv = __insn_v4cmpeq(av_sel, comp_simd);
#else
      FAIL;
#endif
...
#if COMPQ_VLD_BITS == 8
      uint64_t compv = __insn_v1cmpleu(comp_simd, av_sel); 
      uint64_t comp2 = __insn_v1cmpleu(av_sel, comp_simd2); 
#elif COMPQ_VLD_BITS == 16
      uint64_t compv = __insn_v2cmpleu(comp_simd, av_sel); 
      uint64_t comp2 = __insn_v2cmpleu(av_sel, comp_simd2); 
#elif COMPQ_VLD_BITS == 32
      uint64_t compv = __insn_v4cmpleu(comp_simd, av_sel); 
      uint64_t comp2 = __insn_v4cmpleu(av_sel, comp_simd2); 
#else
      FAIL;
#endif
      compv &= comp2;
...
main_aux_worker(void* arg)
{
  ...
  while (1)
  {
    ...
    {
#if WORKER_BATCH_SIZE > 1
      __insn_prefetch(&(idesc[num_copied + 4]));
#endif
      ...
      else 
      {
        ...
        if (idesc[num_copied].be || edesc.c == 3)
        {
#ifdef STATS          
          if (loop_cnt)
          {
        ...
        volatile gxio_mpipe_edesc_t* edesc_ptr;
        edesc_ptr = &(compq_descs[sqn & COMPQ_MASK]);
        edesc_ptr->words[0] = edesc.words[0];
        edesc_ptr->words[1] = edesc.words[1];
      }
    }

    for (int i = 0; i < num_copied; i++)
    {
      // FIXME: Can add optional packet data prefetching here.

      int output_queue_select = handle_packet(&(idesc[i]));

      batch_output_queues[i] = output_queue_select;

      if (output_queue_select == DROP_QUEUE)
      {
      ...
      {
        assert((output_queue_select < num_queues) &&
    ...
    if (num_copied < (WORKER_BATCH_SIZE / 2))
    {
      uint64_t start_cyc = __insn_mfspr(SPR_CYCLE);
...
main(int argc, char** argv)
{
  ...
  for (int i = 0; i < num_links; i++)
  {
    ...
    // Build simple table with input channel mapped to a send queue.
    send_queue_table[channel_num] = i * QUEUES_PER_SEND;
  }
  ...
  tmc_sync_barrier_init(&all_sync_barrier, num_workers + num_senders);
  tmc_spin_barrier_init(&all_spin_barrier, num_workers + num_senders);
  ...
  pthread_t threads[num_workers + num_senders];
  for (int i = 1; i < num_workers; i++)
  {
