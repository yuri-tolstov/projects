...
// The number of entries in the equeue ring - 2048.
...
// The mpipe contexts of ingress and egress links,
// initialized as all NULL.
static gxio_mpipe_context_t* context[LINK_MAX] =
  { NULL };
...
main(int argc, char** argv)
{
  ...
  char* link_name_in = "gbe0";
  char* link_name_out = "gbe16";
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    if ((!strcmp(arg, "--link") || !strcmp(arg, "--links")) &&
        i + 1 < argc)
    {
      // --links <link_name_in> <link_name_out>
      // link_name_in is ingress port, link_name_out is egress port.
      // if only one link is provided, it will be used as ingress and
      // egress port.
      link_name_in = argv[++i];
      link_name_out = link_name_in;
      if (strstr(argv[i + 1], "gbe"))
      {
        link_name_out = argv[++i];
      }
    }
  ...
  // Get the instance.
  instance = gxio_mpipe_link_instance(link_name_in);
  if (instance < 0)
    tmc_task_die("Link '%s' does not exist.", link_name_in);

  // Start the driver.
  result = gxio_mpipe_init(NULL, instance);
  VERIFY(result, "gxio_mpipe_init()");

  // Get the gxio mPIPE context of the instance jsut initalizied.
  context[LINK_INGRESS] = GXIO_MPIPE_CONTEXT(instance);

  gxio_mpipe_link_t link;
  result = gxio_mpipe_link_open(&link, context[LINK_INGRESS], link_name_in, 0);
  ...
  result = gxio_mpipe_alloc_notif_rings(context[LINK_INGRESS], num_workers, 0, 0);
  ...
  for (int i = 0; i < num_workers; i++)
  {
    ...
    result = gxio_mpipe_iqueue_init(iqueue, context[LINK_INGRESS], ring + i,
  ...
  result = gxio_mpipe_alloc_notif_groups(context[LINK_INGRESS], 1, 0, 0);
  ...
  result = gxio_mpipe_alloc_buckets(context[LINK_INGRESS], num_buckets, 0, 0);
  ...
  result = gxio_mpipe_init_notif_group_and_buckets(context[LINK_INGRESS], group,
                                                   ring, num_workers,
  ...
  instance = gxio_mpipe_link_instance(link_name_out);
  result = gxio_mpipe_init(NULL, instance);
  VERIFY(result, "gxio_mpipe_init()");

  context[LINK_EGRESS] = GXIO_MPIPE_CONTEXT(instance);

  if (strcmp(link_name_in, link_name_out))
  {
    result = gxio_mpipe_link_open(&link, context[LINK_EGRESS], link_name_out, 0);
    VERIFY(result, "gxio_mpipe_link_open()");

    channel = gxio_mpipe_link_channel(&link);
  ...
  result = gxio_mpipe_alloc_edma_rings(context[LINK_EGRESS], 1, 0, 0);
  ...
  result = gxio_mpipe_equeue_init(equeue, context[LINK_EGRESS], ering, channel,
                                  edescs, edescs_size, 0);
  VERIFY(result, "gxio_equeue_init()");
  ...
  result = gxio_mpipe_alloc_buffer_stacks(context[LINK_INGRESS],
                                          jumbo ? 3 : 2, 0, 0);
  VERIFY(result, "gxio_mpipe_alloc_buffer_stacks()");
  int stack_idx = result;

  // Initialize small/large stacks.
  create_stack(context[LINK_INGRESS], stack_idx + 0,
               GXIO_MPIPE_BUFFER_SIZE_128,
               num_bufs);
  create_stack(context[LINK_INGRESS], stack_idx + 1,
  ...
  if (jumbo)
  {
    ...
    create_stack(context[LINK_INGRESS], stack_idx + 2, GXIO_MPIPE_BUFFER_SIZE_16384,
                 num_workers * 4);
  ...
  gxio_mpipe_rules_init(&rules, context[LINK_INGRESS]);
