...
int snull_init_module(void) {
   ...
   for (i = 0; i < 2; i++) {
      snull_devs[i] = alloc_netdev(sizeof(struct snull_priv), "sn%d", snull_init);
      if (snull_devs[i] == NULL) {
         printk (KERN_NOTICE "snull: alloc_netdev failed.\n");
         rc = -ENOMEM;
         goto init_module_exit;
      }
      if (register_netdev(snull_devs[i])) {
	 printk(KERN_NOTICE "snull: register_netdev failed\n");
         rc = -ENODEV;
         goto init_module_exit;
      }
   }
