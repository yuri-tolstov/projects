...
static struct mpipe_data {
	...
	/* Is the link status irq enabled? */
	bool link_irq_enabled;
...
static int tile_net_update(struct net_device *dev)
{
	...
	if (!saw_channel) {
		...
		gxio_mpipe_link_close_pollfd(&md->context,
					     md->link_irq_cookie);
		if (md->link_irq_enabled) {
			disable_irq(md->link_irq);
			md->link_irq_enabled = false;
		}
	}
	...
	/* Update the link mask for link status change notification. */
	if (saw_channel) {
		if (md->link_irq_enabled) {
			disable_irq(md->link_irq);	
			md->link_irq_enabled = false;
		...
		on_each_cpu(enable_ingress_irq,
			    (void *)(long)(md->ingress_irq), 1);
		enable_irq(md->link_irq);	
		md->link_irq_enabled = true;
	}
